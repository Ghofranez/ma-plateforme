...
def send_anomaly_alert(
    ...
    scan_badges = {
        "léger":   ("#6366f1", "Scan léger"),
        "rapide":  ("#0891b2", "Scan rapide"),
        "complet": ("#059669", "Scan complet"),
    }
    scan_color, scan_label = scan_badges.get(scan_type, ("#6b7280", scan_type))
    lien_html = _build_rapport_link(analysis_id)

    warning_html = "" if analysis_id else """
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;
      ...
      <div style="background:#1e3a5f;padding:24px;color:white;">
        ...
        <p style="margin:6px 0 0;opacity:.8;font-size:13px;">
          {len(anomalies)} anomalie(s) détectée(s) sur votre URL surveillée
          &nbsp;
          <span style="background:{scan_color};color:white;padding:3px 10px;
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;
      ...
      <div style="background:#1e3a5f;padding:24px;color:white;">
        ...
        <p style="margin:4px 0 0;opacity:.8;font-size:13px;">
          {len(anomalies)} anomalie(s) détectée(s) sur votre site
        </p>
      </div>
      ...
      <div style="padding:24px;">
        ...
        <h3 style="color:#111827;margin-top:24px;margin-bottom:0;">Anomalies détectées</h3>
...
def send_scan_ok_alert(
    ...
    if risk_score is not None:
        if risk_score >= 70:   sc = "#dc2626"
        elif risk_score >= 40: sc = "#ea580c"
        elif risk_score >= 15: sc = "#ca8a04"
        else:                  sc = "#16a34a"
        score_block = f"""
        <div style="margin-top:20px;text-align:center;padding:20px;
                    background:#f9fafb;border-radius:8px;">
          <p style="margin:0;color:#6b7280;font-size:13px;">Score de risque actuel</p>
          <p style="margin:8px 0 0;font-size:48px;font-weight:bold;color:{sc};">
            {risk_score}%
          </p>
        </div>
        """
